...
LOG_FILE="dataeditor-${PROJECT}_${TIMESTAMP}.txt"
...
for TABLE in "$@"; do
  CMD="bq add-iam-policy-binding --member=${MEMBER} --role=roles/bigquery.dataEditor --table=true ${TABLE}"

  echo "Granting roles/bigquery.dataEditor to ${MEMBER} on ${TABLE}"
  echo "$CMD" >> "$LOG_FILE"
